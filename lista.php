...
// ارقام التسلسليه للحذف اذا قمت بحذفه البوت يتوقف عن العمل
if($data == "sendcode"){
...
$inch = file_get_contents("https://api.telegram.org/bot$token/getChatMember?chat_id=@dev_kasper&user_id=".$from_id);
if(strpos($inch , '"status":"left"') !== false ){
bot('sendMessage',['chat_id'=>$chat_id,'parse_mode'=>'Markdown','text'=>"⏬ - اهلا بك عزيزي يجب عليك تقوم القوانين - ⏬\n╣▪اولا يجب الاشتراك في قناة البوت - 🔆\n╣▪معرف قناة البوت : @dev_kasper - 📡\n╣▪اذا لا تشترك البوت لا يعمل ابداً - 🔘\n╣▪اشترك ثمة ارجع ارسل امر /start - 🔅\n╝▪تحياتي لكم : KasperTP - ⚜",'reply_markup'=>json_encode(['inline_keyboard'=>[[['text'=>'قناة البوت', 'url'=>"https://telegram.me/dev_kasper"]],]])]);}
if(!strpos($inch , '"status":"left"') !== false){
...
$msg_id1 = $get->result->message_id;
$msg = $get->result->message_id+1;
$msg_id = $message_id+1;
file_get_contents("https://api.telegram.org/bot$token/deleteMessage?chat_id=$getids[$i]&message_id=$msg_id1");
file_put_contents("data/$msg_id.txt", "$msg\n", FILE_APPEND);
$tp = trim($text, $userbot);
include "code/$tp/lista.php";
$gettxt = file_get_contents("code/$tp/textlist.txt");
$getfile_id = file_get_contents("code/$tp/text.txt");
bot("sendMessage",[
"chat_id"=>$getids[$i],
"text"=>"$gettxt",
'parse_mode'=>HTML,
...
╝▪للاستفسار يرجئ مراسلة المبرمج <a href='https://t.me/$devuser'>$devname</a> - 🎩",
"message_id"=>$message_id,
'reply_markup'=>$ba
]);
}
...
if($text and !$data and $ok > $users[0] and $admins == 1 and !in_array($ch_id, $getids) and !in_array($ch_id, $getban) and $message->chat->type == "supergroup"){
file_put_contents("data/ids.txt", "$ch_id\n", FILE_APPEND);
file_put_contents("start/l$from_id.txt", "");
bot("sendmessage",[
"chat_id"=>$chat_id,
...
if($text and !$data and $ok > $users[0] and $admins != 1 and !in_array($ch_id, $getban) and $message->chat->type == "supergroup"){
bot("sendmessage",[
"chat_id"=>$chat_id,
"text"=>"
⁉️ - عفوا عزيزي البوت لست ادمن - 👁‍🗨
...
if($text and !$data and $ok > $users[0] and $admins == 1 and in_array($ch_id, $getids) and !in_array($json, $getban) and $message->chat->type == "supergroup"){
bot("sendmessage",[
"chat_id"=>$chat_id,
"text"=>"☑️ - القناة مشاركة بتأكيد عزيزي - 👁‍🗨",
"reply_to_message_id"=>$message_id,
...
if($text and !$data and $ok < $users[0] and !in_array($ch_id, $getban) and !in_array($ch_id, $getids) and $message->chat->type == "supergroup"){
bot("sendmessage",[
"chat_id"=>$chat_id,
"text"=>"🚫 - عذرا عدد قناتك قليل - ⁉️",
"reply_to_message_id"=>$message_id,
...
if($text and !$data and $ok > $users[0] and in_array($ch_id, $getban) and $message->chat->type == "supergroup"){
bot("sendmessage",[
"chat_id"=>$chat_id,
"text"=>"🚫 - عذرا قناتك قامة بلمخالفة وتم حظرها - ❌",
"reply_to_message_id"=>$message_id,
...
╝▪اي اقتراح يرجئ مراسلة المبرمج <a href='https://t.me/$devuser'>$devname</a>
",
'parse_mode'=>HTML,
'disable_web_page_preview'=>true,
"message_id"=>$message_id,
...
file_put_contents("code/$message_id/text.txt", $file_id);
file_put_contents("code/$message_id/txt.txt", $txt);
file_put_contents("start/l$from_id.txt", "gooo");
bot('sendmessage',[
'chat_id'=>$chat_id, 
'parse_mode'=>HTML,
...
$msg = $message_id-2;
$ex = explode("\n", $text);
$getfull = file_get_contents("code/$msg/txt.txt");
$gettxt = file_get_contents("code/$msg/textlist.txt");
$getfile_id = file_get_contents("code/$msg/text.txt");
file_put_contents("code/$msg/lista.php", '<?php'."\n".'$list = json_encode(['."\n".'"inline_keyboard"=>['."\n");
...
file_put_contents("code/$msg/lista.php", "\n".'[["text"=>"'.$ch[1].'", "url"=>"'.$ch[2].'"]],', FILE_APPEND);
file_put_contents("code/$msg/code.php", "\n".'[["text"=>"'.$ch[1].'", "url"=>"'.$ch[2].'"]],', FILE_APPEND);
}
if(preg_match("/^(.*) = (.*)\n(.*) = (.*)/", $o, $ch) and $n == 2){
...
'inline_keyboard'=>[
[['text'=>"➕ - صنع منشور - 🔘",'callback_data'=>"start"]],
[['text'=>'🔻 - شارك المنشور - 🔺', 'switch_inline_query'=>"$msg"]],
]])
]);	
}
}